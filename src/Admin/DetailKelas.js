...
const DetailKelas = () => {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        const response = await axios.get(`/api/admin/courses/${id_course}`);
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-lg">
          ...
          {extractYoutubeVideoId(course.course.trailer_video_youtube) ? (
            <iframe
              ...
              src={`https://www.youtube.com/embed/${extractYoutubeVideoId(
                course.course.trailer_video_youtube
              )}`}
              title="Trailer"
              frameBorder="0"
        ...
        <div className="p-5">
          ...
          <p className="mb-3 font-normal text-gray-700">
            <strong>Harga:</strong>{" "}
            {course.course.paid === false
              ? "Gratis"
              : course.course.price
              ? new Intl.NumberFormat("id-ID", {
                  style: "currency",
                  currency: "IDR",
                }).format(course.course.price)
              : ""}
          </p>
          <p className="mb-3 font-normal text-gray-700">
          ...
          <p className="mb-3 font-normal text-gray-700">
            ...
            {course.course.online ? "Online" : "Offline"}
          </p>
          {selectedVideo && (
            <div className="flex justify-center mb-4">
              <iframe
